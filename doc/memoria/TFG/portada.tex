\begin{titlepage}
\begin{center}

%forma de introducir imágenes. el \\[0.5 cm] de final de línea introduce un salto de ese tamaño.
%width=1\textwidth indica el tamaño de la imágen (valores entre 0-1). 
 \includegraphics[width=1\textwidth]{figuras/cabecera.png}  \\[0.5 cm]

\large \textsc{Departamento de Ingeniería de Sistemas y Automática} \\ [1 cm]

\large TRABAJO FIN DE GRADO\\[1 cm]

\huge \textsc{Desarrollo de una plataforma robótica mini-humanoide con visión artificial}\\[7 cm]

%flushleft alinea a la izquierda el texto
\begin{flushleft} \Large
\emph{Autor:} Javier Isabel Hernández\\[0.5 cm]
\emph{Director:} Felix Rodríguez Cañadillas \\
\emph{Tutor:} Alberto Jardón Huete
\end{flushleft}

%rellena de blanco el resto de la página para escribir abajo del todo
\vfill

% Bottom of the page
{\large Leganés, Septiembre 2014}

\end{center}
\end{titlepage}